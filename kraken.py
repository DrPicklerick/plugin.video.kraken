...
playAction = xbmcaddon.Addon('plugin.video.kraken').getSetting('hosts.mode')
autoPlay = 'true' if playAction == '2' else ''
control.homeWindow.setProperty('plugin.video.kraken.autoPlay', autoPlay)
...
if action is None:
	...
	if run == 'true':
		control.execute('RunPlugin(plugin://plugin.video.kraken/?action=tools_cleanSettings)')
...
if action == 'playEpisodesList':
	...
	for i in items:
		...
		url = 'plugin://plugin.video.kraken/?action=play&title=%s&year=%s&imdb=%s&tmdb=%s&tvdb=%s&season=%s&episode=%s&tvshowtitle=%s&premiered=%s&meta=%s&select="2"' % (
...
if action and action.startswith('tools_'):
	if action == 'tools_ShowNews':
		...
		if params.get('opensettings') == 'true':
			control.openSettings(params.get('query2'), 'plugin.video.kraken')

	elif action == 'tools_syncMyAccount':
		control.syncMyAccounts()
		if params.get('opensettings') == 'true':
			control.openSettings(query, 'plugin.video.kraken')
	...
	elif action == 'tools_contextKrakenSettings':
		control.openSettings('0.0', 'context.kraken')
...
if action == 'play':
	...
	# control.player2().play(control.playlist)
	# xbmc.executebuiltin('RunPlugin(plugin://plugin.video.kraken/?action=playAll)')
	# xbmc.executebuiltin('PlayMedia(plugin://plugin.video.kraken/?action=playAll)')
...
if action and action.startswith('cache_'):
	...
	elif action == 'cache_clearKodiBookmark': # context.kraken action call only
		from resources.lib.modules import cache
